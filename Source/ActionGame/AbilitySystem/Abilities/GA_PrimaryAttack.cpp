...
namespace
{
ECollisionChannel GetChannelByName(FName Name)
{
...
void UGA_PrimaryAttack::Shoot_TraceAndCue()
{
	...
	if (bDebugDrawTrace)
	{
		const float DebugDuration = FMath::Max(0.f, DebugDrawDuration);
		DrawDebugLine(Character->GetWorld(), CamLoc, CamEnd, FColor::Blue, false, DebugDuration, 0, 1.0f);
		DrawDebugLine(Character->GetWorld(), MuzzleLoc, MuzzleEnd, bHit ? FColor::Green : FColor::Red, false, DebugDuration, 0, 1.25f);

		if (bCamHit)
		{
			DrawDebugSphere(Character->GetWorld(), AimPoint, 8.f, 10, FColor::Cyan, false, DebugDuration);
		}

		if (bHit && Hit.bBlockingHit)
		{
			DrawDebugSphere(Character->GetWorld(), Hit.ImpactPoint, 12.f, 12, FColor::Yellow, false, DebugDuration);
		}
	}
