...
void AActionGameCharacter::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent)
{
	...
	if (UEnhancedInputComponent* EnhancedInputComponent = Cast<UEnhancedInputComponent>(PlayerInputComponent))
	{
		...
		EnhancedInputComponent->BindAction(MoveAction, ETriggerEvent::Completed, this, &AActionGameCharacter::DoMoveStop);
		EnhancedInputComponent->BindAction(MoveAction, ETriggerEvent::Canceled, this, &AActionGameCharacter::DoMoveStop);
...
void AActionGameCharacter::DoMove(const FInputActionValue& Value)
{
	...
	CachedMoveInputDirection = FVector::ZeroVector;
	...
	const FVector CombinedInput = Forward * MoveInput.Y + Right * MoveInput.X;

	CachedMoveInputDirection = CombinedInput.GetSafeNormal2D();

	if (!HasAuthority())
	{
		ServerUpdateCachedMoveInputDirection(FVector_NetQuantize10(CachedMoveInputDirection));
	}
...
void AActionGameCharacter::PushMoveInputBlock()
{
	++MoveInputBlockCount;
}

void AActionGameCharacter::PopMoveInputBlock()
{
	MoveInputBlockCount = FMath::Max(0, MoveInputBlockCount - 1);
}

void AActionGameCharacter::DoMoveStop(const FInputActionValue& Value)
{
	(void)Value;
	CachedMoveInputDirection = FVector::ZeroVector;
	...
	if (!HasAuthority())
	{
		ServerCommitCachedMoveInputDirection(FVector_NetQuantize10(CachedMoveInputDirection));
	}
}
...
void AActionGameCharacter::SendSkillInputEvent(const FGameplayTag& EventTag)
{
	...
	{
		ServerCommitCachedMoveInputDirection(FVector_NetQuantize10(CachedMoveInputDirection));
		ServerSendSkillInputEvent(EventTag);
	}
...
void AActionGameCharacter::ServerSendSkillInputEvent_Implementation(const FGameplayTag& EventTag)
{
...
{
	CachedMoveInputDirection = FVector(InDirection).GetSafeNormal2D();
}

void AActionGameCharacter::ServerCommitCachedMoveInputDirection_Implementation(const FVector_NetQuantize10& InDirection)
{
	CachedMoveInputDirection = FVector(InDirection).GetSafeNormal2D();
}
