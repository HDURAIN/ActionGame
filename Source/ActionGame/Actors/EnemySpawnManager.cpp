...
bool AEnemySpawnManager::FindSpawnLocation(FVector& OutSpawnLocation) const
{
	...
	if (!NavSystem)
	{
		UE_LOG(LogTemp, Warning, TEXT("EnemySpawnManager: NavigationSystem unavailable."));
		return false;
	}

	const FVector FocusLocation = FocusActor->GetActorLocation();
	const float MinRadiusSq = FMath::Square(FMath::Max(0.f, SpawnRadiusMin));
	const float MaxRadius = FMath::Max(0.f, SpawnRadiusMax);
	...
	for (int32 Attempt = 0; Attempt < MaxAttempts; ++Attempt)
	{
		...
		if (!NavSystem->GetRandomReachablePointInRadius(FocusLocation, MaxRadius, NavLocation))
		{
			continue;
		}

		if (FVector::DistSquared2D(FocusLocation, NavLocation.Location) < MinRadiusSq)
		{
			continue;
		}
	...
	UE_LOG(LogTemp, Warning, TEXT("EnemySpawnManager: Failed to find reachable nav point in spawn radius."));
	return false;
}
...
bool AEnemySpawnManager::SpawnEnemyInternal()
{
	...
	SpawnLocation.Z += (SpawnHeightOffset + SelectedEntry.SpawnHeightOffset);

	if (const UNavigationSystemV1* NavSystem = FNavigationSystem::GetCurrent<UNavigationSystemV1>(World))
	{
		FNavLocation ProjectedLocation;
		const FVector QueryExtent(200.f, 200.f, 600.f);
		if (!NavSystem->ProjectPointToNavigation(SpawnLocation, ProjectedLocation, QueryExtent))
		{
			UE_LOG(LogTemp, Warning, TEXT("EnemySpawnManager: Failed to project spawn location to NavMesh."));
			return false;
		}
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("EnemySpawnManager: NavigationSystem unavailable when projecting spawn point."));
		return false;
	}
